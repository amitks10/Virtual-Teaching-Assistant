...
class FrontEnd:
    ...
    def pre_emphasize(self, wav):
        ...
        for i in range (1,len(wav)):
            temp1=wav[i]-self.preemphasis*wav[i-1]
            preemph_wav.append(temp1)
        return np.array(preemph_wav)

    def wav_to_frames(self, wav):
        # only process whole frames
        num_frames = int(np.floor((np.array(wav).shape[0] - self.win_size) / self.win_shift) + 1)
    ...
    def magspec_to_fbank(self, magspec):
        ...
        pow_frames = ((1.0 / self.fft_size) * ((magspec) ** 2))
        fbank = np.dot(self.mel_filterbank,pow_frames)
    ...
    def mean_norm_fbank(self, fbank):
        ...
        means=fbank.mean(0)
        for i in range (0,fbank.shape[1]):
            for j in range (0,fbank.shape[0]):
                fbank[j][i]=fbank[j][i]-means[i]
